...
Give a project a **color** from its sidebar context menu (Color) or from **Settings → Projects**. The color marks the project's folder icon, puts a stripe on each of its tabs in the sidebar, and shows as a dot beside the window title, so a glance says which project the terminal in front of you belongs to. The eight colors are fixed system colors, so a tag always looks like its name and no two tags collide. **Settings → Appearance → Auto-assign project colors** (off by default) gives each new project the least-used color automatically; it never changes projects you already have.
